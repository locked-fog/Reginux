...
- 将设置浏览器焦点扩展为来源、设置、详情/编辑三个区域；按 Enter 进入下方详情或编辑框，并使用独立高对比激活配色；
- 在当前焦点列表内使用 j/k 导航，避免导航过程中隐式切换焦点。

## 1.1.0 — 2026-08-14

- 重做 TUI 来源浏览：插件/文件双 scope、右侧设置值预览、树状 section 缩进、明确左右焦点和可读的高亮配色；
- Boolean/Enum 使用选项选择器，Raw 编辑打开完整文件；只加载内置 provider 与插件声明来源，并保留不存在的声明文件路径。
